        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    ...
                    hole=0.5,
                    marker=dict(
                        colors=cf.c_colorscale_palette_light  # Use the colors in the order of the labels
                    ),
                    showlegend=False,
                    textinfo='label',
                    textposition='inside',
...
def build_network_graph(df_edges=df_edges, df_nominations=df_nominations):
    ...
    # Set main_category for each person
   ...
   # ========================================================================
    ...
    for node, node_data in G.nodes(data=True):
        ...
        if node_data.get('is_laureate', False):
            hover_lines.insert(3, "*** NOBEL LAUREATE ***")
            hover_lines.insert(4, "")
    ...
    for node_type, style in NODE_STYLES.items():
        ...
        if symbol not in nodes_by_symbol:
            nodes_by_symbol[symbol] = {'x': [], 'y': [], 'text': [], 'color': [], 'size': [], 'ids': []}
    ...
    for node, node_data in G.nodes(data=True):
        ...
        nodes_by_symbol[symbol]['size'].append(style['size'])
    ...
    fig.update_layout(
        ...
        template='plotly_white',
        height=800,
        font=dict(family='Rubik, sans-serif', size=12, color=cf.c_brand_color_main),
        hoverlabel=dict(bgcolor=cf.c_plot_background, font_size=12, font_family="Rubik"),
    )
