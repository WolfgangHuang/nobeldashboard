            ...
            children=[
                        ...
                        dmc.GridCol(
                            dmc.Group(
                                [
                                    ...
                                    html.H1("Nobel Laureate Data Dashboard v0.09.12", className="text-left mt-5 mb-5"),
                                ]
                            ),
                        span=12)
                    ]
                ...
                dmc.Tabs(
                    [
                        dmc.TabsList(
                            [
                                dmc.TabsTab("2024 Prizes", value="tab_2024"),
                                dmc.TabsTab("Quick Facts", value="tab_quickfacts"),
                                dmc.TabsTab("Nationality & Country", value="tab_nationality"),
                                dmc.TabsTab("Gender, Ethnicity & Religion", value="tab_gender"),
                                dmc.TabsTab("Time & Age", value="tab_time"),
                        ...
                        dmc.TabsPanel(
                            dcc.Loading(
                                id="tab-content-loading-2024",  # Unique ID for loading spinner
                                children=html.Div(id="tab-content-2024")  # Unique ID for tab content
                            ),
                            value="tab_2024"
                        ),
                        ...
                        dmc.TabsPanel(
                            dcc.Loading(
                                id="tab-content-loading-quickfacts",
                                children=html.Div(id="tab-content-quickfacts")
                            ),
                            value="tab_quickfacts"
                        ),

                        dmc.TabsPanel(
                            dcc.Loading(
                                id="tab-content-loading-nationality",
                                children=html.Div(id="tab-content-nationality")
                            ),
                            value="tab_nationality"
                        ),

                        dmc.TabsPanel(
                            dcc.Loading(
                                id="tab-content-loading-gender",
                                children=html.Div(id="tab-content-gender")
                            ),
                            value="tab_gender"
                        ),
                    ...
                    value="tab_2024",  # Default selected tab
                    id="tabs",
                ),
            ],
            fluid=True,
...
@app.callback(
    Output('tab-content-2024', 'children'),
    Input('tabs', 'value')
)
def render_tab_content(active_tab):
    if active_tab == 'tab_2024':
        # Load precomputed plots only if they haven't been loaded yet
        with open('pcp_tab_2024.pkl', 'rb') as f1:
            pcp_tab_2024 = pickle.load(f1)
        
        # Extract the figures from the loaded pickle data
        fig_7a = pcp_tab_2024['fig_7a']
        fig_7b = pcp_tab_2024['fig_7b']
...
@app.callback(
    Output('tab-content-quickfacts', 'children'),
    Input('tabs', 'value')
)
def render_tab_content(active_tab):
    if active_tab == 'tab_quickfacts':
        with open('pcp_tab_quickfacts.pkl', 'rb') as f2:
            pcp_tab_quickfacts = pickle.load(f2)
        
        # Extract the figures from the loaded pickle data
        fig_6a = pcp_tab_quickfacts['fig_6a']
        fig_2c = pcp_tab_quickfacts['fig_2c']
        fig_2d = pcp_tab_quickfacts['fig_2d']
        fig_2e = pcp_tab_quickfacts['fig_2e']

        # Return the content for tab Quick Facts
        return dmc.Paper(
            children=[
                dmc.Group(
                    children=[
                        ...
                        html.Div(
                            children=[
                                    ...
                                    html.Div(
                                        f"Exactly {numberofprizes} prizes have been awarded since 1901. Of these, two were declined, and one not received.",
                                        className="description"
                                    )
                            ],
                            className="info-box"
...
@app.callback(
    Output('tab-content-nationality', 'children'),
    Input('tabs', 'value')
)
def render_tab_content(active_tab):
    if active_tab == 'tab_nationality':
        with open('pcp_tab_nationality.pkl', 'rb') as f3:
            pcp_tab_nationality = pickle.load(f3)
        
        # Extract the figures from the loaded pickle data
        fig_1b = pcp_tab_nationality['fig_1b']
        fig_1c = pcp_tab_nationality['fig_1c']
        fig_5a = pcp_tab_nationality['fig_5a']
        fig_5b = pcp_tab_nationality['fig_5b']
        fig_5c = pcp_tab_nationality['fig_5c']
...
def render_tab_content(active_tab):
    if active_tab == 'tab_gender':
        with open('pcp_tab_gender.pkl', 'rb') as f4:
            pcp_tab_gender = pickle.load(f4)
        
        # Extract the figures from the loaded pickle data
        fig_2a = pcp_tab_gender['fig_2a']
        fig_2b = pcp_tab_gender['fig_2b']
        fig_2c = pcp_tab_gender['fig_2c']
        fig_2d = pcp_tab_gender['fig_2d']
        fig_2e = pcp_tab_gender['fig_2e']

        # Return the content for tab Gender
        return dmc.Paper(
            children=[
...
def render_tab_content(active_tab):
    if active_tab == 'tab_time':
        ...
        # Extract the figures from the loaded pickle data
        fig_3a = pcp_tab_time['fig_3a']
        fig_3b = pcp_tab_time['fig_3b']
        fig_3c = pcp_tab_time['fig_3c']
        fig_3d = pcp_tab_time['fig_3d']
...
def render_tab_content(active_tab):
    if active_tab == 'tab_migration':
        ...
        # Extract the figures from the loaded pickle data
        fig_4a = pcp_tab_migration['fig_4a']
        fig_4b = pcp_tab_migration['fig_4b']
...
def render_tab_content(active_tab):
    if active_tab == 'tab_data':
        # with open('pcp_tab_migration.pkl', 'rb') as f:
        #     pcp_tab_migration = pickle.load(f)
        
        # # Extract the figures from the loaded pickle data
        # fig_4a = pcp_tab_migration['fig_3a']
        # fig_4b = pcp_tab_migration['fig_3b']
